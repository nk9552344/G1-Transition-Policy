...
NaN crash fix (training crash at ~500 iters)
──────────────────────────────────────────
The robot was learning to stand (seen at iter 300) but the motion was explosive
and jumpy.  Root cause: MuJoCo PGS solver with iterations=10 could not resolve
the spike contact forces generated by aggressive stand-up attempts (decimation=4
means 20 ms between control steps — large action deltas → large torques).

Fixes applied:
  iterations=20, ls_iterations=30: better constraint convergence under spikes.
  joint_vel_penalty -0.002 → -0.005: damps explosive joint motion.
  action_rate_l2 -0.005 → -0.012: penalises large inter-step action jumps.
  joint_vel_overflow termination at 50 rad/s: resets the exploding episode
    BEFORE NaN propagates to the observation (rsl_rl check_nan crash).
...
def make_recovery_v1_env_cfg() -> ManagerBasedRlEnvCfg:
  ...
  terminations = {
    ...
    "joint_vel_overflow": TerminationTermCfg(
      ...
      params={
        "threshold": 50.0,   # rad/s — physics explosion threshold
        "asset_cfg": SceneEntityCfg("robot"),
      },
    ),
  }
  ...
  return ManagerBasedRlEnvCfg(
    ...
    sim=SimulationCfg(
      ...
      mujoco=MujocoCfg(
        timestep=0.005,
        iterations=20,       # was 10; more PGS iterations = better contact resolution,
                             # prevents constraint-force overflow when robot lands hard
        ls_iterations=30,    # was 20; more line-search iterations = better convergence
        ccd_iterations=50,
      ),
    ),
